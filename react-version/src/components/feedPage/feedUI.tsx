...
const FormSchema = z.object({
  voteOption: z.string().min(1, "Please select an option."),
});
...
function PollForm({ poll }: PollFormProps) {
  const form = useForm<z.infer<typeof FormSchema>>({
    resolver: zodResolver(FormSchema),
  });

  function onSubmit(values: z.infer<typeof FormSchema>) {
    console.log(`Poll ID: ${poll.id} - Selected Option:`, values.voteOption);
                        ...
                        .map((option) => (
                          ...
                          >
                            ...
                            <FormLabel className="font-normal w-full flex justify-between items-center">
                              ...
                              <span className="ml-auto">{option.count}</span>
                            </FormLabel>
                          </FormItem>
                        ))}
                    </RadioGroup>
...
export default function FeedUI({ polls }: FeedUIProps) {
  ...
  return (
    <div className="h-full w-full p-8 grid grid-cols-3 gap-8 auto-rows-[minmax(200px,auto)]">
      {polls.map((poll) => (
        <PollForm poll={poll} key={poll.id} />
      ))}
    </div>
  );
}
